    ...
    def inference_with_vad(self, input, input_len=None, **cfg):
        ...
        for i in range(len(res)):
            ...
            if len(results_sorted) != n:
                ...
                continue
            restored_data = [0] * n
            vad_timestamp = kwargs.get("vad_timestamp", False)
            for j in range(n):
                index = sorted_data[j][1]
                restored_data[index] = results_sorted[j]
                if vad_timestamp and 'timestamp' not in restored_data[index]:
                    # fake timestamp with vad segments
                    restored_data[index]["timestamp"] = []
                    timestamp_len = sorted_data[j][0][1] - sorted_data[j][0][0]
                    text = restored_data[index]["text"]
                    words_count = len(text)
                    match = re.match(r"<.+>", text)
                    if match:
                        words_count -= match.end()
                    timestamp_word = timestamp_len // words_count
                    start = 0
                    for i in range(words_count):
                        end = start + timestamp_word
                        restored_data[index]["timestamp"].append([start, end])
                        start = end
            result = {}
